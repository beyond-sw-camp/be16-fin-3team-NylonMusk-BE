...
<section id="value">
  ...
  <details>
    ...
    <p>
      한국 증권 시장은 표면적으로는 경쟁이 치열한 것처럼 보이지만, 실제로는 
      <strong>IT 인프라가 우월한 대형 증권사 중심의 구조</strong>로 고착화되어 있습니다.
      리테일 시장의 거래 대부분이 상위 소수 증권사에 몰리는 이유가 단순한 브랜드 파워가 아니라,
      각사가 보유한 <strong>주문·체결·정산 시스템의 기술력 차이에서 비롯된 구조적 문제</strong>임이 여러 보고서에서 반복적으로 지적되어 왔습니다.
    </p>
    <p>
      많은 중소형 증권사는 여전히 1990~2000년대 구축된 레거시 시스템을 유지하고 있으며,
      해당 시스템은 복잡한 의존성과 높은 운영 비용 때문에 
      <strong>전면 교체 시 최소 수백억 원의 자본 투입</strong>이 필요합니다.
      금융 IT 교체 주기가 20년 이상 지연되는 대표적 사례로 꼽히는 것이 바로 이 시장입니다.
      그 결과, 대부분의 증권사는 혁신보다 “준비된 기능 안에서 유지보수만 가능한 상태”로 머무르고 있습니다.
    </p>
    <p>
      금융당국은 이러한 문제를 해결하고자 2023년 &lt;중개전문증권사 진입 규제 완화&gt; 정책을 발표했습니다.
      이는 자본금 15억 원만으로도 증권 사업 시작이 가능해졌다는 점에서 큰 관심을 받았습니다.
      그러나 현실은 정반대입니다.
      자본금 요건은 낮아졌지만, <strong>전산 구축 자체는 기존과 동일하게 수십~수백억 원 규모</strong>이기 때문에 
      신규 중개사가 실질적으로 시장에 진입하기는 거의 불가능한 상황입니다.
    </p>
    <p>
      또 하나의 문제는 <strong>비표준화된 운영 구조</strong>입니다.
      각 증권사는 주문 시스템, 계좌 시스템, 공모·배정 시스템, 재무 공시, 리스크 관리 등
      상호 연동되어야 하는 기능들을 개별적으로 구축해 운영하고 있습니다.
      이 때문에 시스템 간 데이터 정합성이 깨지기 쉽고,
      증권업이 본질적으로 갖춰야 할 <strong>투명성·추적성·감사 가능성</strong>이 기술 구조상 제한됩니다.
    </p>
    <p>
      요약하자면, 한국 증권 시장에는 다음과 같은 구조적 문제가 존재합니다:
      <ul>
        <li>① 레거시 시스템 유지 → 중소형사는 기술 경쟁력 확보 불가</li>
        <li>② 신규 중개사는 전산 구축 비용 부담으로 진입 불가</li>
        <li>③ 주문·정산·공시·심사 시스템이 분절되어 정보 단절 발생</li>
        <li>④ 기업·증권사·투자자가 사용하는 데이터 기준이 제각각</li>
      </ul>
      이러한 문제는 단순한 UI의 불편함을 넘어, 
      <strong>시장 경쟁력과 공정성 자체를 저해하는 근본적 기술 장벽</strong>으로 작동하고 있습니다.
    </p>
  </details>
  ...
  <details>
    ...
    <p>
      MKX는 이러한 구조적 문제를 해결하기 위해 고안된 
      <strong>B2B 입점형 디지털 증권 거래 플랫폼</strong>입니다.
      기존 증권사들이 개별적으로 구축하던 주문·체결·정산·공모·위험 감시 시스템을 하나의 기술 스택으로 통합하고, 
      이를 <strong>SaaS 형태로 제공</strong>함으로써 시장의 기술 격차를 해소합니다.
    </p>
    <p>
      증권사는 MKX에 입점하는 순간,
      자체 시스템 구축 없이 <strong>거래소 수준의 인프라에 즉시 연결</strong>됩니다.
      이를 통해 신규 증권사나 중소형 증권사는
      기존의 수백억 원 규모의 초기 투자 없이도
      <strong>주문 서버, 매칭엔진, 실시간 차트, 공모·배정 모듈</strong>을 즉시 사용할 수 있습니다.
    </p>
    <p>
      사용자는 회원가입 시 자신이 선택할 증권사를 지정하고,
      해당 증권사 계정으로 MKX의 트레이딩 인프라를 이용하게 됩니다.
      이 구조는 다음과 같은 두 가지 효과를 만듭니다:
      <ul>
        <li><strong>① 다수의 증권사가 입점 → 자연스럽게 사용자 유입 증가</strong></li>
        <li><strong>② 증권사는 MKX 인프라 활용 → 전산 구축 부담 완화</strong></li>
      </ul>
    </p>
    <p>
      MKX가 제공하는 가장 큰 차별성은
      <strong>“증권 업무의 Full Supply Chain”</strong>을 단일 데이터 모델로 구현했다는 점입니다.
      주문·체결 데이터가 정산·공모·위험감시로 이어지며,
      어떤 기능에서도 데이터 단절이 발생하지 않습니다.
      이는 기존 전통 증권사조차 해결하지 못한 문제입니다.
    </p>
    <p>
      결과적으로 MKX는
      <strong>중소형 증권사의 진입 장벽을 파괴하는 인프라</strong>이자,
      <strong>투명한 운영 기반을 제공하는 차세대 디지털 증권 플랫폼</strong>입니다.
      단순한 거래 시스템이 아니라,
      <strong>기업·증권사·투자자 모두를 하나의 생태계로 연결하는 새로운 시장 구조</strong>를 제안합니다.
    </p>
  </details>
</section>
